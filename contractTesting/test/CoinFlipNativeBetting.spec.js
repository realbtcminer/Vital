    ...
    describe("#requestRandomWords", async function () {
        describe("success", async function () {
            it("Should successfully request a random number", async function () {
                const { coinFlip, VRFCoordinatorV2Mock, deployer } = await loadFixture(
                ...
                await expect(playTX).to.emit(VRFCoordinatorV2Mock, "RandomWordsRequested")
            ...
            it("Should successfully take the bet, save the VRFFee information", async function () {
                const { coinFlip, deployer } = await loadFixture(deployRandomNumberConsumerFixture)

                const balanceBeforeTx = await ethers.provider.getBalance(coinFlip.address)
                const wager = 1
                ...
                const balanceAfterTx = await ethers.provider.getBalance(coinFlip.address)
                const VRFFees = await coinFlip.VRFFees()
                expect(VRFFees).gt(ethers.constants.Zero)
                expect(balanceAfterTx.sub(balanceBeforeTx).sub(VRFFees)).to.eq(wager)
            })

            it("Should successfully request a random number and get a result", async function () {
                const { deployer, coinFlip, VRFCoordinatorV2Mock } = await loadFixture(
                    deployRandomNumberConsumerFixture
                )
                const wager = 100
                const playTX = await coinFlip.CoinFlip_Play(
                ...
                const state = await coinFlip.CoinFlip_GetState(deployer.address)
                const requestId = state.requestID

                //simulate callback from the oracle network
                await expect(
                    VRFCoordinatorV2Mock.fulfillRandomWords(requestId, coinFlip.address)
                ).to.emit(coinFlip, "CoinFlip_Outcome_Event")
            })

            it("Should correctly calculate the award and make the payment", async function () {
                const { player, deployer, coinFlip, VRFCoordinatorV2Mock } = await loadFixture(
                    deployRandomNumberConsumerFixture
                )
                const wager = 100
                const numGames = 5
                ...
                const events = await coinFlip.queryFilter("CoinFlip_Outcome_Event")
                ...
                await expect(payoutTX).changeEtherBalance(player.address, payout)
                await expect(payoutTX).changeEtherBalance(coinFlip.address, -payout)
            })

            it("Should stop games when stopGain is reached", async function () {
                const { player, coinFlip, VRFCoordinatorV2Mock } = await loadFixture(
                    deployRandomNumberConsumerFixture
                )
                const wager = 100
                const stopGain = 150
                ...
                //simulate callback from the oracle network
                const payoutTX = await VRFCoordinatorV2Mock.fulfillRandomWordsWithOverride(
                    requestId,
                    coinFlip.address,
                    [1, 1, 1, 1, 1]
                )
                ...
                await expect(payoutTX).changeEtherBalance(
                    player.address,
                    (numGames - gamesPlayed) * wager + (gamesPlayed * 198 * wager) / 100
                )
            })
            ...
            it("Should work properly CoinFlip_Refund()", async function () {
                const { player, coinFlip } = await loadFixture(deployRandomNumberConsumerFixture)
                const wager = 100
                const stopGain = 150
                const stopLoss = 201
                const numGames = 5
                ...
                const firstRefundTx = coinFlip.connect(player).CoinFlip_Refund()
                ...
                const secondRefundTx = coinFlip.connect(player).CoinFlip_Refund()
                await expect(secondRefundTx).changeEtherBalance(player.address, wager * numGames)
                await expect(secondRefundTx).changeEtherBalance(coinFlip.address, -wager * numGames)
