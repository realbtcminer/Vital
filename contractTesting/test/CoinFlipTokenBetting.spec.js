    ...
    describe("#requestRandomWords", async function () {
        describe("success", async function () {
            it("Should successfully request a random number", async function () {
                const { coinFlip, VRFCoordinatorV2Mock, token, deployer, player } =
            ...
            it("Should successfully take the bet, save the VRFFee information", async function () {
                ...
                const tokenBalanceBeforeTx = await token.balanceOf(coinFlip.address)
                const wager = 1
                await coinFlip
                    .connect(player)
                ...
                const tokenBalanceAfterTx = await token.balanceOf(coinFlip.address)
                const VRFFees = await coinFlip.VRFFees()
                expect(VRFFees).gt(ethers.constants.Zero)
                expect(balanceAfterTx.sub(balanceBeforeTx).sub(VRFFees)).to.eq(0)
                expect(tokenBalanceAfterTx.sub(tokenBalanceBeforeTx)).to.eq(wager)
            })

            it("Should correctly calculate the award and make the payment", async function () {
                const { player, token, coinFlip, VRFCoordinatorV2Mock } = await loadFixture(
                    deployRandomNumberConsumerFixture
                )
                const wager = 100
                const numGames = 5
                const playTX = await coinFlip
                    .connect(player)
                    .CoinFlip_Play(wager, token.address, true, numGames, 10000, 10000, {
                        value: ethers.utils.parseEther("1"),
                    })
                ...
                await expect(payoutTX).to.changeTokenBalance(token, player.address, payout)

                await expect(payoutTX).to.changeTokenBalance(token, coinFlip.address, -payout)
            })
            ...
            it("Should stop games when stopGain is reached", async function () {
                ...
                const playTX = await coinFlip
                    .connect(player)
                    .CoinFlip_Play(wager, token.address, true, numGames, stopGain, stopLoss, {
                        value: ethers.utils.parseEther("1"),
                    })
                const state = await coinFlip.CoinFlip_GetState(player.address)
                const requestId = state.requestID
            ...
            it("Should stop games when stopLoss is reached", async function () {
                const { player, coinFlip, token, VRFCoordinatorV2Mock } = await loadFixture(
                    deployRandomNumberConsumerFixture
                )
                const wager = 100
                const stopGain = 150
