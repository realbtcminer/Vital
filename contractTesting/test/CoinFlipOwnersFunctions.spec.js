    ...
    describe("#testOwnersFunctions", async function () {
        describe("success", async function () {
            it("Should successfully transfer Ownership", async function () {
                ...
                const notAnOwnerTX = coinFlip.connect(player).transferOwnership(player.address)

                const ownerTX = coinFlip.connect(deployer).transferOwnership(player.address)
            ...
            it("Should successfully change Subscription", async function () {
                ...
                const notAnOwnerTX = coinFlip.connect(player).changeSubscription(2)

                const ownerTX = coinFlip.connect(deployer).changeSubscription(2)

                await expect(notAnOwnerTX).be.reverted

                await expect(ownerTX).to.emit(coinFlip, "Subscrition_Change_Event").withArgs(1, 2)
            ...
            it("Should successfully set bet Token", async function () {
                ...
                const notAnOwnerTX = coinFlip
                    .connect(player)
                    .setToken("0x0000000000000000000000000000000000000001", true)
                ...
                await expect(ownerTX)
                    ...
                    .withArgs("0x0000000000000000000000000000000000000001", true)

                expect(
                    await coinFlip.isTokenAllowed("0x0000000000000000000000000000000000000001")
                ).to.eq(true)
            })
            ...
            it("Should successfully withdraw House Edge", async function () {
                ...
                await expect(ownerTX).changeEtherBalance(player.address, withdrawAmount)
            })
        })
    })
})
